...
- **前端是单文件**：全部 HTML/CSS/JS 都在 `my-task-desktop/src/index.html`（约 5230 行），改 UI 只动这一个文件。
...
- `ux-simplification-plan.md` — 交互简化优化方案（2026-09-11 产出），改造包 A/B（2 个）+ C/D/E（3 个）；**§9 实施记录已记录全部落地情况、偏差与顺带修掉的 3 个 bug**；§10~§15 是 2026-09-15 的七轮用户反馈
- `ui-spec.md` — **界面规范（标准本身，不是决策过程）**，2026-09-15 制定，当前只覆盖弹窗：尺寸阶梯 / 结构 / 排版 / 按钮变体与页脚顺序 / 危险色规则 / 18 个弹窗的档位分配 / 改动检查清单。**改弹窗先读它。** 别的模块（列表、侧栏、看板）将来要立规范也放这里
